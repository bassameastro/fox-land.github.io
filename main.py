...
def get_random_profile_photo():
    """Select a random profile photo from the directory."""
    profile_pics = os.listdir(PROFILE_PIC_DIR)
    return os.path.join(PROFILE_PIC_DIR, random.choice(profile_pics))
...
def message(data):
    ...
    profile_photo = get_random_profile_photo()  # Assign a random profile photo

    content = {
        "name": user_name,
        "message": data["data"],
        "profilePhoto": profile_photo  # Include the profile photo URL
    }

    # Send message to the specified room
    send(content, to=room)

    # Append message to the room's message history
